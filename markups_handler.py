...
def gotopay1(user_id):
    ...
    payurl = pay.gen_invoice_url(490, 'Оплата сервиса Ronk.VPN сроком на 1 месяц', order_id)
...
def gotopay2(user_id):
    ...
    order_id = mysql.select_formed_order_id_in_orders(user_id)['MAX(id)']
    #Формируем ссылку для оплаты
    payurl = pay.gen_invoice_url(1250, 'Оплата сервиса Ronk.VPN сроком на 3 месяца', order_id)
...
def gotopay3(user_id):
    ...
    order_id = mysql.select_formed_order_id_in_orders(user_id)['MAX(id)']
    #Формируем ссылку для оплаты
    payurl = pay.gen_invoice_url(4990, 'Оплата сервиса Ronk.VPN сроком на 12 месяцев', order_id)
